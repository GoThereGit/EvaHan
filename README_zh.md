...
- 封闭模式（强制）：仅允许使用官方训练数据 + 允许的预训练视觉 - 语言模型（基准线基于Qwen2.5_VL_7B_Instruct     https://www.modelscope.cn/models/Qwen/Qwen2.5-VL-7B-Instruct 或者 [Xunzi_Qwen2_VL_7B_Instruct (https://huggingface.co/RAY5/Xunzi_Qwen2_VL_7B_Instruct)）
...
- 最终提交格式: 测试集A、B、C，分别对应训练集A、B、C，按照前面指定的格式，分别提交三个对应的json文件，命名为"TestA.json"、"TestB.json"、"TestC.json";
同时，还需要提交一个说明性文档，简要说明训练方法、使用的模型和数据等信息。最后，将以上四个文件打包到一个文件夹中，命名格式:编号+队长名+参赛模式。
